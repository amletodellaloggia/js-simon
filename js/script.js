...
function startCountdown() {
  countdownEl.textContent = timeLeft; // Mostro il valore di timeLeft nella pagina
...
form.addEventListener("submit", function (e) {
  ...
  for (let i = 0; i < userNumbers.length; i++) { // Scorro userNumbers
    ...
    let trovatoNeiGenerati = false; // Dichiaro e setto variabile a false per controllo
    for (let j = 0; j < generatedNumbers.length; j++) { // Scorro
      if (userNum === generatedNumbers[j]) { //Se userNum è uguale a generatedNumbers in posizione [j] allora la variabile diventa true
        trovatoNeiGenerati = true;
      }
    }

    // Controllo se userNum è già stato indovinato (mi servo di k)
    let giàIndovinato = false; // Dichiaro e setto variabile a false per controllo
    for (let k = 0; k < guessed.length; k++) { // Scorro
      if (userNum === guessed[k]) { // Se userNum è uguale a guessed in posizione [k] allora la variabile diventa true
        giàIndovinato = true;
      }
    }

    // Se è nei numeri generati e non è già stato aggiunto, lo pusho in guessed
    if (trovatoNeiGenerati === true && giàIndovinato === false) { 
